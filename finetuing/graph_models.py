...
LastEditTime: 2021-10-21 20:52:00
...
from build_tree import build_icd9_tree, build_atc_tree, build_px_tree
...
class ConcatEmbeddings_4(nn.Module):
    ...
    def __init__(self, config, dx_voc, px_voc, item_voc, rx_voc):
        ...
        self.special_embedding = nn.Parameter(
            torch.Tensor(config.code_vocab_size-len(dx_voc.idx2word)-len(rx_voc.idx2word)-len(px_voc.idx2word)-len(item_voc.idx2word),  \
    ...
    def forward(self, input_ids):
        emb = torch.cat(
            [self.special_embedding, self.dx_embedding(), self.px_embedding(), self.item_embedding, self.rx_embedding()], dim=0)
